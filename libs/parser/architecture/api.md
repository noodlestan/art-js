...
2. **Visit nodes** — each mdast node is visited in tree order:
   - **Processors run first** — any construct with a `processor` hook is consulted. If one returns a Construct record, the node is claimed and the factory layer is skipped.
...
- **ConstructParser** — the union of three optional hooks: `processor`, `factory`, `handler`.
- **ConstructProcessor** — claims a node immediately, bypassing factory detection.
